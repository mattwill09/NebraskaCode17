...
namespace InAppPurchaseTest
{
	public partial class PurchaseTableCell : UITableViewCell
	{
		...
		public void DisplayProduct(UIViewController controller, InAppProduct product) {
			...
			case InAppProductType.NonConsumable:
				if (Product.Downloadable) {
					...
					if (Product.NewContentAvailable) {
						...
						UpdateButton.Hidden = InAppPurchaseHandler.PurchaseManager.DownloadInProgress;
			...
			if (!_wiredUp) {
				...
				UpdateButton.TouchUpInside += (object sender, EventArgs e) => {
					...
					if (_displayContent) {
						...
						// Attach to product
						content.manager = InAppPurchaseHandler.PurchaseManager;
						...
						if (InAppPurchaseHandler.PurchaseManager.SimulateiTunesAppStore && !InAppPurchaseHandler.PurchaseManager.SimulatedRestoredPurchaseProducts.Contains(Product.ProductIdentifier)) {
							// No, quickly add it to the list of products to restore
							// This is a hack for testing simulated downloads 
							if (InAppPurchaseHandler.PurchaseManager.SimulatedRestoredPurchaseProducts == "") {
								InAppPurchaseHandler.PurchaseManager.SimulatedRestoredPurchaseProducts = Product.ProductIdentifier;
							} else {
								InAppPurchaseHandler.PurchaseManager.SimulatedRestoredPurchaseProducts +=  "," + Product.ProductIdentifier;
							}
						}
						...
						// already has.
						InAppPurchaseHandler.PurchaseManager.RestorePreviousPurchases();
					}
				};
